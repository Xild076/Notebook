...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Settings">
      {/* Tab Navigation */}
      <div className="flex border-b border-[#404040] mb-4">
      ...
      <div className="space-y-6 max-h-[70vh] overflow-y-auto">
        ...
        {activeTab === 'general' && (
          <>
            {/* Theme Setting */}
            <div>
              <label className="block text-sm font-medium text-[#dcddde] mb-1">
                Theme
              </label>
              <select
                value={theme}
                onChange={(e) => setTheme(e.target.value as 'obsidian' | 'light' | 'dark')}
                className="w-full px-3 py-2 bg-[#2d2d2d] border border-[#404040] rounded-md shadow-sm focus:outline-none focus:ring-[#7c3aed] focus:border-[#7c3aed] text-[#dcddde]"
              >
