...
export const Editor: React.FC<EditorProps> = ({ content, onChange, showStatusBar = true }) => {
  ...
  return (
    ...
    >
          ...
          {block.type === 'website' && (
            ...
            >
              {rawModeBlocks.has(block.id) ? (
                <textarea 
                  className="w-full h-full p-4 font-mono text-sm bg-[#1e1e1e] text-[#dcddde] resize-none outline-none border-none"
                  value={block.content}
      ...
      {/* Status Bar */}
      {showStatusBar && (
        <div className="shrink-0 flex items-center justify-between px-6 py-2 bg-[#262626] border-t border-[#404040] text-xs text-[#888]">
